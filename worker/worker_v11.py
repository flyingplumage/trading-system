#!/usr/bin/env python3
"""Iris Worker v11 - 硬件监控 + 更多日志"""
import os, json, time, threading, subprocess, socket, asyncio, websockets, requests, shutil, psutil
...
worker_state = {
    ...
    "worker_version": "v11",
    "hardware": {"cpu_percent": 0, "memory_percent": 0, "memory_used": 0, "memory_total": 0, "disk_percent": 0}
}
...
def get_hardware_info():
    ...
    try:
        ...
        return {"cpu_percent": 0, "memory_percent": 0, "memory_used": 0, "memory_total": 0, "disk_percent": 0}
...
async def hardware_monitor():
    """硬件监控线程"""
    while True:
        worker_state["hardware"] = get_hardware_info()
        state_changed.set()
...
<head>
    ...
    <title>Iris Worker v11 - {{ worker_id }}</title>
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .progress-fill.memory { background: linear-gradient(90deg, #8b5cf6, #6d28d9); }
        .progress-fill.disk { background: linear-gradient(90deg, #ec4899, #be185d); }
        .bottom-grid { display: grid; grid-template-columns: 2fr 3fr; gap: 12px; }
        .install-list { max-height: 250px; overflow-y: auto; }
        ...
        .logs-container { background: rgba(0,0,0,0.3); border-radius: 10px; padding: 12px; max-height: 350px; overflow-y: auto; font-family: "SF Mono", Monaco, monospace; font-size: 11px; }
        ...
        .hw-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 8px; margin-top: 8px; }
        .hw-item { text-align: center; padding: 8px; background: rgba(0,0,0,0.2); border-radius: 6px; }
        .hw-value { font-size: 16px; font-weight: 600; margin-bottom: 4px; }
        .hw-label { font-size: 10px; color: #64748b; }
    </style>
</head>
<body>
    <div class="container">
        <header>
            <div>
                <h1>🚀 Iris Worker <span id="version-badge" class="badge badge-info">v11</span></h1>
        ...
        <div class="top-grid">
            ...
            <div class="card"><h2>💻 硬件</h2>
                <div class="hw-grid">
                    <div class="hw-item"><div class="hw-value" id="cpu">0%</div><div class="hw-label">CPU</div></div>
                    <div class="hw-item"><div class="hw-value" id="memory">0%</div><div class="hw-label">内存</div></div>
                    <div class="hw-item"><div class="hw-value" id="disk">0%</div><div class="hw-label">磁盘</div></div>
                </div>
            </div>
            <div class="card"><h2>📦 依赖</h2>
        ...
        <div class="bottom-grid">
            ...
            <div class="card">
                <h2>📝 日志 <span style="color:#64748b;font-size:11px;">(最近 100 条)</span></h2>
    ...
    <script>
        ...
        function updateUI(s) {
            document.getElementById('version-badge').textContent = s.worker_version || 'v11';
            ...
            const hw = s.hardware || {};
            document.getElementById('cpu').textContent = (hw.cpu_percent || 0) + '%';
            document.getElementById('memory').textContent = (hw.memory_percent || 0) + '%';
            document.getElementById('disk').textContent = (hw.disk_percent || 0) + '%';
        ...
        function renderLogs() {
            ...
            logsContainer.innerHTML = logs.slice(-100).reverse().map(log => {
