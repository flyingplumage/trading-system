...
<body>
    <div class="container">
        <header>
            <div>
                <h1>🚀 Iris Worker <span id="version-badge" class="badge badge-info">v15</span></h1>
    ...
    <script>
        ...
        eventSource.addEventListener('message',function(event){try{updateUI(JSON.parse(event.data));}catch(e){console.error(e);}});
        ...
        function updateUI(s){
            document.getElementById('version-badge').textContent=s.worker_version||'v15';
        ...
        function renderLineChart(type,data){
            ...
            const width=300,height=80,stepX=width/(data.length-1);
            const points=data.map((value,index)=>{const x=index*stepX;const y=height-(value/100*height);return x+','+y;}).join(' ');
