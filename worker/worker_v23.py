...
<html lang="zh-CN">
<head><meta charset="UTF-8"><title>Iris Worker v23</title>
...
async def upload_file_chunked(file_path, task_id, chunk_size=1024*1024):
    ...
    add_log("info", f"📤 开始上传文件：{file_path} ({file_size/1024/1024:.2f}MB, {total_chunks} 块)")
        ...
        with open(file_path, 'rb') as f:
            for chunk_id in range(total_chunks):
                ...
                if ws_connection:
                    ...
                    add_log("progress", f"📤 上传进度：{chunk_id+1}/{total_chunks} ({(chunk_id+1)/total_chunks*100:.1f}%)")
                    await asyncio.sleep(0.1)  # 避免发送太快
        
        add_log("success", f"✅ 文件上传完成：{file_path}")
        return True
    except Exception as e:
        add_log("error", f"❌ 文件上传失败：{e}")
        return False
