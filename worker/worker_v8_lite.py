#!/usr/bin/env python3
"""Iris Worker v8 Lite - 轻量级 SSE 实时推送（不刷新页面）"""
...
def add_log(level, message):
    ...
    try:
        log_queue.put_nowait({"timestamp": datetime.now().isoformat(), "level": level, "message": message})
    except:
        pass
                ...
                async for message in ws:
                    ...
                    try:
                        ...
                        if msg_type == "install_dependencies":
                            ...
                            asyncio.create_task(install_dependencies_smart(data.get("packages", [])))
                        elif msg_type == "train":
...
def stream():
    def generate():
        while True:
            yield f"data: {json.dumps({'type': 'status', 'data': worker_state})}\n\n"
            time.sleep(1)
...
<head>
    ...
    <title>Iris Worker v8 Lite - {{ worker_id }}</title>
    <style>
...
if __name__ == "__main__":
    ...
    print(f"Iris Worker v8 Lite 启动\nWebSocket: {WS_URL}\n监控：http://localhost:{WORKER_PORT}/")
