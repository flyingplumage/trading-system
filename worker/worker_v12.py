...
<head>
    ...
    <title>Iris Worker v12 - {{ worker_id }}</title>
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .hw-chart { height: 60px; display: flex; align-items: flex-end; gap: 2px; padding: 5px 0; }
        .hw-bar { flex: 1; background: linear-gradient(180deg, var(--chart-color), transparent); border-radius: 2px 2px 0 0; min-width: 3px; transition: height 0.3s ease; }
        .hw-bar.cpu { --chart-color: #3b82f6; }
        .hw-bar.memory { --chart-color: #8b5cf6; }
        .hw-bar.disk { --chart-color: #ec4899; }
        .hw-current { font-size: 18px; font-weight: 600; text-align: center; margin-bottom: 8px; }
...
<body>
    <div class="container">
        <header>
            <div>
                <h1>🚀 Iris Worker <span id="version-badge" class="badge badge-info">v12</span></h1>
    ...
    <script>
        ...
        function updateUI(s) {
            document.getElementById('version-badge').textContent = s.worker_version || 'v12';
        ...
        function renderChart(type, data) {
            ...
            const maxBars = 30;
            const bars = data.slice(-maxBars).map(value => {
                const height = Math.min(value, 100);
                return `<div class="hw-bar ${type}" style="height:${height}%"></div>`;
            }).join('');
