...
ws_connection = None
message_log = []
current_operation = None  # 'installing' or 'training'
                ...
                async for message in ws:
                    ...
                    try:
                        ...
                        if msg_type == "install_dependencies":
                            ...
                            asyncio.create_task(upgrade_worker(data))
...
async def install_dependencies(packages):
    ...
    worker_state["dependency_progress"] = 0
    add_log("info", f"📦 开始安装依赖：{len(packages)} 个包")
    
    total = len(packages)
    for i, pkg in enumerate(packages):
        try:
            cmd = ["pip", "install", "-i", "https://pypi.tuna.tsinghua.edu.cn/simple", pkg, "-q"]
            subprocess.run(cmd, capture_output=True, text=True, timeout=300)
            worker_state["dependency_progress"] = int((i + 1) / total * 100)
            add_log("success", f"✅ 已安装：{pkg}")
        except Exception as e:
            add_log("error", f"❌ 安装失败：{pkg} - {e}")
    
    worker_state["dependencies_installed"] = True
    worker_state["status"] = "idle"
    current_operation = None
    add_log("success", "✅ 所有依赖安装完成")
...
def health():
    return jsonify({
        ...
        "worker_id": socket.gethostname(),
        "websocket": worker_state["websocket_connected"],
        "dependencies_installed": worker_state["dependencies_installed"],
        "uptime": worker_state["uptime_seconds"]
    })
...
def status():
    return jsonify({
        ...
        "uptime_seconds": worker_state["uptime_seconds"],
        "current_operation": current_operation,
        "start_time": worker_state["start_time"]
    })
...
<body>
    <div class="container">
        <header>
            <h1>🚀 Iris Worker</h1>
            <div class="subtitle">{{ worker_id }} • ws://{{ server_ip }}:5000/ws</div>
        </header>
        
        <div class="grid">
            <div class="card">
                <h2>📊 连接状态</h2>
                <div class="stat-row"><span class="stat-label">WebSocket</span><span class="stat-value" id="ws-status">...</span></div>
                ...
                <div class="stat-row"><span class="stat-label">发送消息</span><span class="stat-value" id="msg-sent">0</span></div>
            </div>
            
            <div class="card">
                <h2>📈 运行状态</h2>
                <div class="stat-row"><span class="stat-label">当前状态</span><span class="stat-value" id="status">...</span></div>
                <div class="stat-row"><span class="stat-label">当前操作</span><span class="stat-value" id="operation">无</span></div>
                <div class="stat-row"><span class="stat-label">依赖状态</span><span class="stat-value" id="deps-status">...</span></div>
    ...
    <script>
        async function update() {
            try {
                ...
                document.getElementById('msg-sent').textContent = s.messages_sent || 0;
