...
worker_state = {
    ...
    "training_progress": 0, "training_loss": 0, "training_epoch": 0,
                ...
                async for message in ws:
                    ...
                    try:
                        ...
                        if msg_type == "install_dependencies":
                            ...
                            asyncio.create_task(install_dependencies_smart(data.get("packages", []), data.get("source", "pip")))
                        elif msg_type == "train":
...
async def install_dependencies_smart(packages, preferred_source="pip"):
...
<head>
    ...
    <title>Iris Worker v6 - {{ worker_id }}</title>
    ...
    <script>
        async function update() {
            try {
                ...
                document.getElementById('epoch-tag').textContent = s.training_epoch ? 'Epoch ' + s.training_epoch : '';
...
if __name__ == "__main__":
    ...
    print(f"Iris Worker v6 Final 启动\nWebSocket: {WS_URL}\n监控：http://localhost:{WORKER_PORT}/")
