#!/bin/bash
# Iris Worker v31 一键部署脚本
# WebSocket 信令 + HTTP 文件传输 - 优化布局（左侧卡片，右侧日志）

set -e

SERVER_IP="${SERVER_IP:-162.14.115.79}"
WS_PORT="${WS_PORT:-5000}"
WORK_DIR=~/iris-worker-v31

echo "============================================================"
echo "🚀 Iris Worker v31 一键部署"
echo "============================================================"
echo ""
echo "架构：WebSocket 信令 + HTTP 文件传输"
echo "特性：优化布局（左侧卡片，右侧日志）"
...
# 验证版本
VERSION=$(grep "worker_version.*v31" worker.py && echo "v31" || echo "unknown")
if [ "$VERSION" != "v31" ]; then
    echo "⚠️  警告：下载的版本可能不是 v31"
fi
...
docker build -t iris-worker:v31 . 2>&1 | tail -10
echo "✅ 镜像构建完成"
