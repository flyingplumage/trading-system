...
# HTML 模板（纯 ASCII，避免编码问题）
HTML_TEMPLATE = '''<!DOCTYPE html>
<html lang="zh-CN">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
    <title>Iris Worker v24</title>
    <style>
        :root{--bg:#0f172a;--card:#1e293b;--text:#f1f5f9}
        *{margin:0;padding:0;box-sizing:border-box}
        body{font-family:-apple-system,BlinkMacSystemFont,"Microsoft YaHei",sans-serif;background:var(--bg);color:var(--text);padding:15px}
        .container{max-width:1800px;margin:0 auto}
        header{margin-bottom:15px}
        .top-grid{display:grid;grid-template-columns:repeat(5,1fr);gap:12px;margin-bottom:15px}
        .card{background:var(--card);border-radius:10px;padding:15px}
        .card h2{font-size:12px;margin-bottom:12px}
        .hw-chart{height:80px;margin-top:10px}
        .hw-line polyline{fill:none;stroke-width:1.5}
        .hw-line.cpu polyline{stroke:#3b82f6}
        .hw-line.memory polyline{stroke:#8b5cf6}
        .hw-line.disk polyline{stroke:#ec4899}
        .hw-current{text-align:center;margin-bottom:5px}
        .hw-label{text-align:center;font-size:10px}
        .bottom-grid{display:grid;grid-template-columns:2fr 1fr;gap:12px}
        .logs-container{max-height:600px;overflow-y:visible;font-family:monospace;font-size:11px}
...
<body>
    <div class="container">
        <header>
            <h1>Iris Worker v24</h1>
            <div id="subtitle">Loading...</div>
        </header>
        <div class="top-grid">
            <div class="card"><h2>Connection</h2><div id="ws-status">-</div></div>
            <div class="card"><h2>Status</h2><div id="op-status">-</div></div>
            <div class="card"><h2>CPU</h2><div class="hw-current" id="cpu-current">0%</div><div class="hw-chart cpu"><svg viewBox="0 0 300 80"><polyline id="cpu-polyline" points=""></polyline></svg></div></div>
            <div class="card"><h2>Memory</h2><div class="hw-current" id="memory-current">0%</div><div class="hw-chart memory"><svg viewBox="0 0 300 80"><polyline id="memory-polyline" points=""></polyline></svg></div></div>
            <div class="card"><h2>Disk</h2><div class="hw-current" id="disk-current">0%</div><div class="hw-chart disk"><svg viewBox="0 0 300 80"><polyline id="disk-polyline" points=""></polyline></svg></div></div>
        </div>
        <div class="bottom-grid">
            <div class="card"><h2>Logs</h2><div class="logs-container" id="logs"></div></div>
            <div class="card"><h2>Dependencies</h2><div id="install-details"></div></div>
        </div>
    </div>
    <script>
        const eventSource = new EventSource('/stream');
        eventSource.onopen = () => loadInitialData();
        eventSource.addEventListener('message', e => updateUI(JSON.parse(e.data)));
        function loadInitialData() {
            fetch('/status').then(r => r.json()).then(updateUI);
            fetch('/logs').then(r => r.json()).then(d => renderLogs(d.logs));
        }
        function updateUI(s) {
            document.getElementById('subtitle').textContent = s.worker_id + ' - ws://' + s.worker_id;
            document.getElementById('ws-status').textContent = s.websocket_connected ? 'Connected' : 'Disconnected';
            document.getElementById('op-status').textContent = s.status || '-';
            document.getElementById('cpu-current').textContent = (s.hardware?.cpu_percent || 0) + '%';
            document.getElementById('memory-current').textContent = (s.hardware?.memory_percent || 0) + '%';
            document.getElementById('disk-current').textContent = (s.hardware?.disk_percent || 0) + '%';
            ...
            document.getElementById('install-details').innerHTML = (s.dependency_install_log || []).map(i => i.package + ' ' + (i.success ? 'OK' : 'Wait')).join('<br>');
        }
        function renderChart(t, d) {
            const p = document.getElementById(t + '-polyline');
            if (!p || d.length < 2) return;
            const w = 300, h = 80, s = w / (d.length - 1);
            p.setAttribute('points', d.map((v, i) => (i * s) + ',' + (h - (v / 100 * h))).join(' '));
        }
        function renderLogs(logs) {
            document.getElementById('logs').innerHTML = (logs || []).slice(-100).reverse().map(l => '[' + new Date(l.timestamp).toLocaleTimeString() + '] [' + l.level + '] ' + l.message).join('<br>');
        }
    </script>
</body>
</html>'''
...
async def upload_file_chunked(file_path, task_id, chunk_size=1024*1024):
    if not os.path.exists(file_path):
        add_log("error", f"File not found: {file_path}")
        return False
    file_size = os.path.getsize(file_path)
    total_chunks = (file_size + chunk_size - 1) // chunk_size
    add_log("info", f"Uploading file: {file_path} ({file_size/1024/1024:.2f}MB, {total_chunks} chunks)")
    try:
        with open(file_path, 'rb') as f:
            for chunk_id in range(total_chunks):
                ...
                if ws_connection:
                    ...
                    add_log("progress", f"Upload: {chunk_id+1}/{total_chunks} ({(chunk_id+1)/total_chunks*100:.1f}%)")
                    await asyncio.sleep(0.1)
        add_log("success", f"File upload complete: {file_path}")
        return True
    except Exception as e:
        add_log("error", f"File upload failed: {e}")
        return False
...
async def execute_training(task):
    ...
    for epoch in range(epochs):
        ...
        for progress in range(0, 101, 10):
            ...
            state_changed.set()
    model_path = f"/app/models/{task_id}.bin"
    if os.path.exists(model_path):
        add_log("info", "Starting model upload...")
        await upload_file_chunked(model_path, task_id)
    worker_state["status"] = "idle"
