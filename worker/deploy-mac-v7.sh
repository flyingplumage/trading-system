#!/bin/bash
# Iris Worker v7 Mac 一键部署脚本
# 特性：依赖明细 + Epoch 进度 + 50 条日志
...
echo "============================================================"
echo "🚀 Iris Worker v7 一键部署"
echo "============================================================"
echo ""
echo "新功能："
echo "  📦 依赖安装明细（每个包的尝试源/使用源）"
echo "  🧠 Epoch 进度条（显示所有 Epoch 完成情况）"
echo "  📝 50 条日志显示（缓存 500 条）"
echo ""

# 配置镜像加速
mkdir -p ~/.docker
cat > ~/.docker/daemon.json << 'MIRROR'
{"registry-mirros":["https://docker.m.daocloud.io","https://docker.1panel.live","https://hub.rat.dev"]}
MIRROR
...
# 下载代码
WORK_DIR=~/iris-worker-v7
mkdir -p "$WORK_DIR"
cd "$WORK_DIR"
curl -sSL "http://$SERVER_IP:8888/worker_v7.py" -o worker.py
...
echo ""
echo "🎯 v7 新功能:"
echo "   📦 依赖安装明细（包名/尝试源/使用源）"
echo "   🧠 Epoch 进度（所有 Epoch 完成情况）"
echo "   📝 50 条日志显示（可滚动查看 500 条）"
echo ""
