...
@app.route('/')
def index():
    resp = make_response(render_template_string(HTML_TEMPLATE, worker_id=socket.gethostname(), server_ip=SERVER_IP))
...
<head>
    ...
    <title>Iris Worker v14 - {{ worker_id }}</title>
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .bottom-grid { display: grid; grid-template-columns: 1fr 2fr; gap: 12px; }
        ...
        .logs-container { background: rgba(0,0,0,0.3); border-radius: 10px; padding: 12px; max-height: 500px; overflow-y: auto; font-family: "SF Mono", Monaco, monospace; font-size: 11px; }
        ...
        .hw-chart { height: 80px; position: relative; border-left: 1px solid #475569; border-bottom: 1px solid #475569; margin-top: 10px; }
        .hw-line { position: absolute; bottom: 0; left: 0; width: 100%; height: 100%; }
        .hw-line path { fill: none; stroke-width: 2; stroke-linecap: round; stroke-linejoin: round; }
        .hw-line.cpu path { stroke: #3b82f6; }
        .hw-line.memory path { stroke: #8b5cf6; }
        .hw-line.disk path { stroke: #ec4899; }
...
<body>
    <div class="container">
        <header>
            <div>
                <h1>🚀 Iris Worker <span id="version-badge" class="badge badge-info">v14</span></h1>
        ...
        <div class="top-grid">
            ...
            <div class="card"><h2>💻 CPU</h2>
                ...
                <div class="hw-chart cpu"><svg class="hw-line cpu" id="cpu-svg" viewBox="0 0 300 80"></svg></div>
                <div class="hw-label">最近 60 秒</div>
            </div>
            <div class="card"><h2>🧠 内存</h2>
                <div class="hw-current" id="memory-current">0%</div>
                <div class="hw-chart memory"><svg class="hw-line memory" id="memory-svg" viewBox="0 0 300 80"></svg></div>
                <div class="hw-label">最近 60 秒</div>
            </div>
            <div class="card"><h2>💾 磁盘</h2>
                <div class="hw-current" id="disk-current">0%</div>
                <div class="hw-chart disk"><svg class="hw-line disk" id="disk-svg" viewBox="0 0 300 80"></svg></div>
    ...
    <script>
        ...
        function updateUI(s) {
            document.getElementById('version-badge').textContent = s.worker_version || 'v14';
            ...
            renderLineChart('cpu', s.hardware_history?.cpu || []);
            renderLineChart('memory', s.hardware_history?.memory || []);
            renderLineChart('disk', s.hardware_history?.disk || []);
        ...
        function renderLineChart(type, data) {
